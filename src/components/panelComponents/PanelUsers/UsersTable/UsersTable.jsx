...
export default function TicketsTable() {
  return (
    <div className="border overflow-hidden dark:border-gray-600 rounded-2xl">
        ...
        <TableBody className="bg-white text-black font-Kalame text-center dark:bg-[#1B2631] dark:text-white">
          {invoices.map((invoice) => (
            ...
            >
              ...
              <TableCell className="text-right">{invoice.userName}</TableCell>
              ...
              <TableCell className="text-center w-[180px]">
                {invoice.wallet === "0" ? (
                  <div className="text-center bg-red-500 rounded-lg w-[6rem] px-1 text-white font-bold">
                    بدون موجودی
                  </div>
                ) : (
                  <div className="text-center rounded-lg px-1 w-[6rem] bg-green-500 text-white font-bold">
                    {invoice.wallet}
                  </div>
                )}
              </TableCell>
              ...
              <TableCell className="text-right flex items-center justify-center gap-1">
                <NavLink
                  activate
                  to={`/panel/users/${invoice.id}`}
                  className="bg-blue-400 p-1 w-7 h-7 rounded-full"
                >
                  <svg
